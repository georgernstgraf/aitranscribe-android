...
After changing Hilt dependency graph (e.g., adding `PostProcessTextUseCase` to `TranscriptionWorker`, adding `OpenRouterApiService` DI), incremental builds can produce APKs missing `AITranscribeApp_GeneratedInjector`. **Fix:** Always run `./gradlew clean assembleDebug` before deploying to device. The `HiltSmokeTest` instrumentation test verifies the generated class exists.
